...
button{margin-left:8px;padding:12px 18px;border-radius:20px;border:none;background:#00c851;color:#fff;font-weight:bold}
</style></head>
<body>
<div id="chat"><div class="b">Oi! Sou o Porteiro IA. Como posso ajudar?</div></div>
<div id="bar"><input id="inp" placeholder="Digite..."><button onclick="send()">ENVIAR</button></div>
<script>
async function send(){
 let i=document.getElementById('inp'); let t=i.value.trim(); if(!t)return;
 let c=document.getElementById('chat'); c.innerHTML+=`<div class="u">Você: ${t}</div>`; i.value='';
 try{
  let r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:t})});
  let d=await r.json();
  c.innerHTML+=`<div class="b">IA: ${d.reply}</div>`;
...
def chat():
    try:
        ...
        comp = client.chat.completions.create(
            ...
            messages=[{"role":"system","content":"Você é o Porteiro IA, simpático e prestativo, fale em pt-br"},{"role":"user","content":user_msg}],
            max_tokens=400
        )
